...
public class FragmentCart extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		((MainActivity) getActivity()).mAbCheckout.setVisibility(View.VISIBLE);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		TextView subtotallabel = (TextView) mParentView
				.findViewById(R.id.fra_cart_tv_subtotal_label);
		...
		if (CartManager.getInstance().getdeliverydouble() <= 0) {
			vatLabel.setVisibility(View.GONE);
			vatValue.setVisibility(View.GONE);
		} else {
			vatLabel.setText(getString(R.string.fra_cart_vat));
		}

		if (CartManager.getInstance().getVatDouble() <= 0) {
		...
		((Button)mParentView.findViewById(R.id.ordrbtn)).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				
				((MainActivity) getActivity()).showCartFragmentPreview(true, false);
				//
	...
	public void generateValues() {
		...
		String CURRENCY="";
		savedValue.setText(CartManager.getInstance().getSubTotalString()
				+ CURRENCY);
		
		subtotalValue.setText(CartManager.getInstance().getVatString()
				+ CURRENCY);
		
		vatValue.setText(CartManager.getInstance().getdeliveryString()
				+ CURRENCY);
